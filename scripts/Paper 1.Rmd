...
# define seasons
winter <- (rainfall_L$month == "Jun" | rainfall_L$month == "Jul" | rainfall_L$month == "Aug")
spring <- (rainfall_L$month == "Sep" | rainfall_L$month == "Oct" | rainfall_L$month == "Nov")
earlysummer <- (rainfall_L$month == "Dec")
latesummer <- (rainfall_L$month == "Jan" | rainfall_L$month == "Feb")
autumn <- (rainfall_L$month == "Mar" | rainfall_L$month == "Apr" | rainfall_L$month == "May")
...
rainfall_L$season <- factor(rainfall_L$season , levels=c('EarlySummer','LateSummer', 'Autumn', 'Winter', 'Spring'))
...
earlysummer <- (rainfall$month == "Dec")
latesummer <- (rainfall$month == "Jan" | rainfall$month == "Feb")
...
cum.co2fluxes <- input.rearranged %>% #input.daily %>%
  select(NEE_LT.gC, GPP_LT.gC, ER_LT.gC, dayssinceJul1, GSYear) %>% 
  group_by(GSYear) %>% 
...
# define seasons
winter <- (input.rearranged$month == "Jun" | input.rearranged$month == "Jul" | input.rearranged$month == "Aug")
spring <- (input.rearranged$month == "Sep" | input.rearranged$month == "Oct" | input.rearranged$month == "Nov")
earlysummer <- (input.rearranged$month == "Dec")
latesummer <- (input.rearranged$month == "Jan" | input.rearranged$month == "Feb")
autumn <- (input.rearranged$month == "Mar" | input.rearranged$month == "Apr" | input.rearranged$month == "May")

input.rearranged$season[winter] <- "Winter"
input.rearranged$season[spring] <- "Spring"
input.rearranged$season[earlysummer] <- "EarlySummer"
input.rearranged$season[latesummer] <- "LateSummer"
input.rearranged$season[autumn] <- "Autumn"

input.rearranged$season <- factor(input.rearranged$season , levels=c('EarlySummer','LateSummer', 'Autumn', 'Winter', 'Spring'))
```

```{r}
rainfall.SP <- input.rearranged %>% select(Precip, month, GSYear, season)


rainfall <- rainfall_L %>% select(Precip, month, GSYear, season) %>% rbind(., rainfall.SP)
