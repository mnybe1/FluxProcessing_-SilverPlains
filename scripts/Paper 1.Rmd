...
colnames(rainfall_L)[3] <- "day"
colnames(rainfall_L)[2] <- "month"
rainfall_L$Site <- "L"
rainfall_L$date <- as.Date(with(rainfall_L, paste(Year, month, day,sep="-")), "%Y-%m-%d")
rainfall_L$jday <- yday(rainfall_L$date)
...
rainfall.SP$Site <- "SP"
rainfall.SP$date <- as.Date(with(rainfall.SP, paste(Year, month, jday,sep="-")), "%Y-%m-%j")
rainfall.2merge <- rainfall_L %>% filter(Year == 2019  & jday >= 244 & jday < 354)



rainfall <- rbind.fill(rainfall.2merge, rainfall.SP)
...
```{r create new dataframe for timeseries analysis}
rainfall.2merge <- rainfall %>% filter(Year == 2019  & jday < 354)
input.daily$Site <- "SP"

tsData <- rbind.fill(rainfall.2merge, input.daily) %>% 
  select(Year, month, jday, Precip, NEE_LT.gC, ER_LT.gC, GPP_LT.gC) %>%
  mutate(
    NEE2weeks = lag(NEE_LT.gC, n = 14, default = NA)
  )

tsData$date <- as.Date(with(tsData, paste(Year, month, jday,sep="-")), "%Y-%m-%d")
lm1 <- lm(NEE2weeks~Precip, tsData)
summary(lm1)

lm2 <- lm(NEE_LT.gC~Precip, tsData)
summary(lm2)
```
