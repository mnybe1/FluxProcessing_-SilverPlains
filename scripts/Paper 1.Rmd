...
rainfall.summary <- rainfall %>% ddply(c("Year", "Site", "season"), summarise,
                                       seasonal.sum = sum(Precip, na.rm = TRUE))

rainfall.summary %>% 
  ggplot(., aes(x = season, y = seasonal.sum, group = Year, fill = Site)) +
...
```
```{r seasonal NEE}
NEE.summary <- input.daily %>% ddply(c("Year", "season", "jday"), summarise,
                                       seasonal.sum = cumsum(NEE_LT.gC))
