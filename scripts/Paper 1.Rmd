...
(SWC.20 <- input.rearranged %>% filter (GSYear == "19/20" ) %>% ggplot(., aes(x = as.Date(dayssinceJun1, origin = as.Date("06-01", "%m-%d")), SWC, group = GSYear)) +
  geom_line(aes(color = GSYear), size = 1.5)+
  # scale_x_continuous(limits = c(0, 365))+
  xlab('Month') +
  ylab('SWC %') +
   scale_x_date(breaks = "month", date_labels = "%b", limits = as.Date(c("06-01", "05-31"), "%m-%d"))+
...
NEE.summary$month.name <- factor(NEE.summary$month.name, levels=c("Jul", "Aug", "Sep", "Oct", "Nov", "Dec", "Jan", "Feb", "Mar", "Apr", "May", "Jun"))
...
(NEEcum.p <-cum.co2fluxes %>% 
  ggplot(., aes(x = as.Date(dayssinceJul1, origin = as.Date("07-01", "%m-%d")), NEEcum, color = GSYear)) +
  geom_line(size = 1)+
...
(GPPcum.p <-cum.co2fluxes %>% 
   ddply(c("GSYear", "season"), summarise,
  seasonal.total = cumsum(GPP_LT.gC)) %>% filter(season == "Winter") %>%
   ggplot(., aes(x = GSYear, y = seasonal.total, group = GSYear, fill = season)) +
  geom_col() + 
  ylab("Total GPP")+
  xlab("Growing season year")+
...
GPPcum.p
tiff("GPP.tiff", units="in", width=10, height=6,  res=300)
print(GPPcum.p)
dev.off()

(ERcum.p <-cum.co2fluxes %>% 
   ddply(c("GSYear", "season"), summarise,
  seasonal.total = cumsum(ER_LT.gC)) %>% filter(season == "Summer") %>%
   ggplot(., aes(x = GSYear, y = seasonal.total, group = GSYear, fill = season)) +
  geom_col() + 
       scale_y_continuous(limits = c(0,60000))+
  ylab("Total ER")+
...
ERcum.p
tiff("ER.tiff", units="in", width=10, height=6,  res=300)
print(ERcum.p)
dev.off()
```
