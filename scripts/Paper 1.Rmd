...
rainfall_L$Site <- "L"
rainfall_L <- rainfall_L %>% filter(month >= 5)
...
Seasonal.rain <- rainfall%>% filter(Year == 2020 | Year == 2021 | Year == 2022) %>% ddply(c("Year", "season"), summarise,
                    seasonal.sum = sum(Precip, na.rm = TRUE))

rain.aov <- aov(seasonal.sum~Year*season, data =Seasonal.rain)
summary(rain.aov)
