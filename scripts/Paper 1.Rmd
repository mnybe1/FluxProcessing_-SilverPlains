...
cumulative.co2fluxes <- input %>%
  drop_na(Timestamp) %>% # some NAs in the Timestamp for some reason. Need to look into
  ddply(c("Year", "jday", "month", "season"), summarise,
        Fco2 = gofC.f(mean(Fco2, na.rm = TRUE)),
        ER = gofC.f(mean(ER_SOLO, na.rm = TRUE)),
        GPP = gofC.f(mean(GPP_SOLO, na.rm = TRUE)))
