...
summer <- (input.r$month.name == "Dec" | input.r$month.name == "Jan" | input.r$month.name == "Feb" | input.r$month.name =="Mar")
autumn <- (input.r$month.name == "Apr" |input.r$month.name == "May")
...
GPP.summary<- cum.co2fluxes %>% dplyr::select(GPP_LT.gC,dayssinceJun1, GSYear, season) %>% 
...
ER.summary<- cum.co2fluxes %>% dplyr::select(ER_LT.gC,dayssinceJun1, GSYear, season) %>% 
  group_by(GSYear, season) %>% 
   ddply(c("GSYear", "season"), summarise,
...
monthlycumsum <- input.r %>%
  dplyr:: select(NEE_LT.gC, GPP_LT.gC, ER_LT.gC, dayssinceJun1, GSYear, season, month) %>% 
  group_by(GSYear, month) %>% 
...
FFP <- input %>% dplyr::select(Timestamp, Year, month,L,WS, sigma_v,  WD, u_star)
...
FFP <- FFP[, c(1, 2, 3, 9,10, 6, 11, 12, 13, 4, 8, 5,7)] # change variable order to comply with data format

write.csv(FFP, "C:/Users/Marion/Documents/UTas/FluxProcessing_SilverPlains/FFP.csv")

meanmm  <- FFP %>% dplyr::select(zm, d, L, z0, u_mean, sigma_v, wind_dir, u_star, mm, day, HH_UTC, MM) %>% ddply(c("mm","day", "HH_UTC", "MM"), summarise,
                                                                                                 zm = mean(zm), 
                                                                                                 d = mean(d),
                                                                                                 z0 = mean(z0),
                                                                                                 u_mean = mean(u_mean),
...
```{r Carbon uptake period}
summer.input <- input.r %>% filter (season == "Summer" | season =="Autumn")

cum.co2fluxes.sum <- summer.input%>%
  dplyr::select(NEE_LT.gC, GPP_LT.gC, ER_LT.gC, dayssinceJun1, GSYear, season) %>% 
  group_by(GSYear) %>% 
  mutate(NEEcum = cumsum(NEE_LT.gC),
         GPPcum = cumsum(GPP_LT.gC),
         ERcum = cumsum(ER_LT.gC))
...
(NEEcum.p <-cum.co2fluxes.sum %>% 
  ggplot(., aes(dayssinceJun1, NEEcum, group = GSYear, color = GSYear)) +
  geom_line(size = 1, aes(color = GSYear))+
   # scale_color_manual(values = group.colour)+
       scale_x_continuous(limits = c(184, 365))+
  ylab(bquote('Cumulative NEE (g C ' ~m^-2~')'))+
  xlab("Days since June 1st")+
  theme_classic()+ 
    theme(axis.title = element_text(size = 14),
...
CO2uptakesum <- summer.input %>% filter(NEE_LT.gC<0)
CO2uptakesum.1920 <- CO2uptakesum %>% filter(GSYear == '19/20') #49
CO2uptakesum.2021 <- CO2uptakesum %>% filter(GSYear == '20/21') #101
CO2uptakesum.2122 <- CO2uptakesum %>% filter(GSYear == '21/22') #107
